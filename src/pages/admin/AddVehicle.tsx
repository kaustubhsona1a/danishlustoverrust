    ...
    <form onSubmit={handleSubmit} className="space-y-8 max-w-4xl font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2.5 w-full sm:w-auto">
          ...
          <button type="submit" className="flex-grow sm:flex-grow-0 text-center px-4 sm:px-6 py-3.5 bg-[#00C0FF] hover:bg-white text-zinc-950 rounded-xl text-xs font-bold tracking-widest font-mono uppercase transition-all shadow-lg shadow-[#00C0FF]/10">{isEditing ? 'Save Changes' : 'Save Vehicle'}</button>
        </div>
      </div>
      ...
      <div className="bg-zinc-950/65 backdrop-blur-md p-4 sm:p-6 md:p-8 rounded-2xl border border-white/5 shadow-2xl space-y-8 text-zinc-300">
        ...
        {/* Media */}
        <div>
          <h2 className="text-sm font-bold font-serif text-[#00C0FF] mb-6 border-b border-white/5 pb-2 uppercase tracking-widest">Vehicle Gallery</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {images.map((img, i) => (
              <div 
                ...
                className={`relative aspect-video rounded-xl overflow-hidden border ${draggedIdx === i ? 'border-[#00C0FF] opacity-50' : 'border-white/5'} group cursor-move`}
              >
            ...
            <label className="border-2 border-dashed border-white/10 rounded-xl aspect-video flex flex-col items-center justify-center p-4 text-center bg-zinc-900/20 hover:bg-[#00C0FF]/5 text-zinc-400 hover:text-[#00C0FF] transition-all cursor-pointer font-mono text-xs">
          ...
          <div className="mt-4 p-4 rounded-xl border border-white/5 bg-zinc-900/10 space-y-2">
            <label className="text-[9px] uppercase font-mono tracking-wider text-zinc-400 font-bold block">Or Add Image via URL</label>
