                  ...
                  {leads.map(lead => (
                    <tr key={lead.id} className="hover:bg-white/5 transition-colors font-mono">
                          ...
                          <div className="flex gap-2 mt-2.5 pt-2 border-t border-white/5 flex-wrap">
                            {lead.images.map((img, idx) => (
                              <button 
                                ...
                                className="block h-12 w-12 rounded-lg overflow-hidden border border-white/10 hover:border-[#00C0FF] transition-colors bg-zinc-900 group"
                              >
                      ...
                      <td className="px-6 py-4">
                        <select 
                          ...
                          className={`px-3 py-1.5 rounded text-[9px] font-bold uppercase tracking-wider border outline-none cursor-pointer bg-zinc-950 focus:border-[#00C0FF] transition-all ${
                            lead.status === 'New Lead' ? 'border-[#00C0FF]/25 text-[#00C0FF]' :
                            lead.status === 'Contacted' ? 'border-zinc-700 text-zinc-400' :
                            lead.status === 'Negotiating' ? 'border-purple-500/20 text-purple-400' :
                            'border-zinc-800 text-zinc-500'
                          }`}
                        >
                          <option value="New Lead" className="bg-zinc-950 text-[#00C0FF]">New Lead</option>
                      ...
                      <td className="px-6 py-4 text-right space-x-3">
                        <a href={`tel:${lead.phone}`} className="text-[10px] text-[#00C0FF] hover:text-white font-semibold font-mono uppercase tracking-wider transition-colors border border-[#00C0FF]/25 hover:border-white px-2 py-1 rounded">Call</a>
                        <button 
                ...
                <div key={lead.id} className="p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-3 pt-2">
                    ...
                    <div className="flex items-center space-x-2 shrink-0">
                      <a 
                        href={`tel:${lead.phone}`} 
                        className="px-3.5 py-2 bg-[#00C0FF]/10 hover:bg-[#00C0FF] border border-[#00C0FF]/25 text-[#00C0FF] hover:text-zinc-950 rounded-lg text-[10px] font-extrabold uppercase tracking-widest transition-all"
                      >
                        Call
                      </a>
                      <button 
            ...
            <div className="space-y-4">
              ...
              <div className="text-center sm:text-left space-y-2">
                ...
                <div className="bg-white/[0.02] border border-white/5 rounded-lg p-3 text-xs text-zinc-400 text-left font-mono">
                  <span className="text-[9px] uppercase tracking-wider text-[#00C0FF] block mb-1">Lead Interest</span>
          ...
          <div className="bg-zinc-950 border border-white/10 rounded-2xl relative z-10 shadow-2xl max-w-3xl w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-t border-white/5 bg-zinc-900/20 text-center flex justify-between items-center">
              ...
              <button 
                ...
                className="text-[10px] text-[#00C0FF] hover:text-white font-mono uppercase tracking-widest font-bold transition-all border border-[#00C0FF]/25 hover:border-white py-1.5 px-3 rounded-lg"
              >
                Download File ↗
              </button>
            </div>
