...
import { UploadCloud, Trash2, Plus, Image as ImageIcon, Link as LinkIcon, AlertCircle } from 'lucide-react';
import { uploadImageToStorage, cleanupLegacyImageVariants } from '../../lib/supabase';
...
export default function AdminSettings() {
  ...
  const [customHeroMobileUrl, setCustomHeroMobileUrl] = useState('');
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, key: 'logo' | 'homeHeroImage' | 'homeHeroMobileImage' | 'aboutImage') => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        const labelText = key === 'logo' ? 'Logo' : key === 'homeHeroImage' ? 'Home Hero Image' : key === 'homeHeroMobileImage' ? 'Home Hero Mobile Image' : 'About Image';
        setSuccess(`${labelText} updated successfully!`);
        setTimeout(() => setSuccess(''), 3000);
      } catch (err: any) {
        console.error('Image upload failed', err);
        setErrorText(err.message || 'Failed to process custom image upload.');
        setTimeout(() => setErrorText(''), 5000);
      } finally {
  ...
  const handleSaveUrl = (key: 'logo' | 'homeHeroImage' | 'homeHeroMobileImage' | 'aboutImage', url: string) => {
    if (url.trim()) {
      updateSiteConfig({ [key]: url.trim() });
      const labelText = key === 'logo' ? 'Logo URL' : key === 'homeHeroImage' ? 'Hero Background URL' : key === 'homeHeroMobileImage' ? 'Hero Mobile Background URL' : 'About Image URL';
      setSuccess(`${labelText} updated successfully!`);
      setTimeout(() => setSuccess(''), 3000);
      if (key === 'logo') setCustomLogoUrl('');
      if (key === 'homeHeroImage') setCustomHeroUrl('');
      if (key === 'homeHeroMobileImage') setCustomHeroMobileUrl('');
  ...
  const handleAddDeliveryUrl = (e: React.FormEvent) => {
    e.preventDefault();
    if (deliveryUrl.trim()) {
      const current = siteConfig.clientDeliveries || [];
      updateSiteConfig({ clientDeliveries: [...current, deliveryUrl.trim()] });
      setDeliveryUrl('');
      setSuccess('Client delivery photo added via URL!');
      setTimeout(() => setSuccess(''), 3000);
    }
  };
  ...
  return (
    <div className="space-y-8 max-w-4xl font-sans text-zinc-300">
      ...
      <div className="bg-zinc-950/65 backdrop-blur-md rounded-2xl border border-white/5 shadow-2xl p-4 sm:p-6 md:p-8 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-sm font-bold font-serif text-[#00C0FF] mb-1 uppercase tracking-widest">Storage Optimization</h2>
          <p className="text-zinc-500 text-[10px] uppercase font-mono tracking-wider">Clean up legacy vehicle image variants (-thumb, -gallery, -full) from storage to save space.</p>
        </div>
        <button 
          onClick={handleCleanupLegacyVariants}
          disabled={isCleaning}
          className="bg-red-500/20 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/30 font-bold px-6 py-3 rounded-xl text-xs uppercase tracking-widest font-mono transition-all flex items-center shrink-0 disabled:opacity-50 w-full md:w-auto justify-center"
        >
          <Trash2 className="w-4 h-4 mr-2" /> 
          {isCleaning ? 'Cleaning...' : 'Purge Legacy Variants'}
        </button>
      </div>
      ...
      <div className="bg-zinc-950/65 backdrop-blur-md rounded-2xl border border-white/5 shadow-2xl p-4 sm:p-6 md:p-8 space-y-10">
        ...
        {/* Logo Section */}
        <div>
          <h2 className="text-sm font-bold font-serif text-[#00C0FF] mb-1 uppercase tracking-widest">Showroom Brand Logo</h2>
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-start gap-6">
            ...
            <div className="flex-grow space-y-4">
              <label className="block w-full cursor-pointer bg-zinc-900/25 border-2 border-dashed border-white/10 hover:border-[#00C0FF] hover:bg-[#00C0FF]/5 rounded-xl p-6 transition-all text-center">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-grow text-xs px-4 py-3 border border-white/5 bg-zinc-950 rounded-xl text-white outline-none placeholder-zinc-700 focus:border-[#00C0FF] transition-all font-mono"
                />
                <button 
                  type="button"
                  onClick={() => handleSaveUrl('logo', customLogoUrl)}
                  className="bg-[#00C0FF]/25 hover:bg-[#00C0FF] text-[#00C0FF] hover:text-zinc-950 border border-[#00C0FF]/30 font-bold px-4 py-3 rounded-xl text-xs uppercase tracking-widest font-mono transition-all flex items-center shrink-0"
                >
                  <Plus className="w-3.5 h-3.5 mr-1" /> Use URL
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Home Hero Image */}
        <div>
          <h2 className="text-sm font-serif font-bold text-[#00C0FF] mb-1 uppercase tracking-widest">Home Page Background Image</h2>
          <p className="text-zinc-500 text-[10px] uppercase font-mono tracking-wider mb-6">Changes the main full-screen luxury background backdrop in the home landing screen.</p>
          <div className="flex flex-col md:flex-row items-stretch md:items-start gap-6">
            <div className="w-52 aspect-video overflow-hidden rounded-xl border border-white/5 bg-zinc-900/30 shrink-0 relative shadow-sm">
              <img src={sanitizeHeroImage(siteConfig.homeHeroImage)} alt="Home Hero Preview" className="w-full h-full object-cover" />
            </div>
            <div className="flex-grow space-y-4">
              <label className="block w-full cursor-pointer bg-zinc-900/25 border-2 border-dashed border-white/10 hover:border-[#00C0FF] hover:bg-[#00C0FF]/5 rounded-xl p-6 transition-all text-center">
                ...
                <UploadCloud className="w-8 h-8 text-zinc-500 mx-auto mb-2" />
                <p className="text-xs font-bold text-white font-mono uppercase tracking-wider">
                  {isCompressing ? 'Compacting Showroom Backdrop...' : 'Upload Showroom Backdrop'}
                </p>
                <p className="text-[10px] text-zinc-550 font-mono mt-0.5 uppercase tracking-wider">Wide landscape image (automatically compressed & optimized)</p>
              </label>
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  placeholder="Or paste direct backdrop background image URL..."
                  className="flex-grow text-xs px-4 py-3 border border-white/5 bg-zinc-950 rounded-xl text-white outline-none placeholder-zinc-700 focus:border-[#00C0FF] transition-all font-mono"
                />
                <button 
                  type="button"
                  onClick={() => handleSaveUrl('homeHeroImage', customHeroUrl)}
                  className="bg-[#00C0FF]/25 hover:bg-[#00C0FF] text-[#00C0FF] hover:text-zinc-950 border border-[#00C0FF]/30 font-bold px-4 py-3 rounded-xl text-xs uppercase tracking-widest font-mono transition-all flex items-center shrink-0"
                >
                  <Plus className="w-3.5 h-3.5 mr-1" /> Use URL
                </button>
        ...
        <div>
          <div className="flex items-center space-x-2 mb-1">
            <h2 className="text-sm font-serif font-bold text-[#00C0FF] uppercase tracking-widest">Home Page Mobile Background Image</h2>
            <span className="bg-[#00C0FF]/15 text-[#00C0FF] text-[8px] font-bold font-mono px-2 py-0.5 rounded tracking-wider uppercase border border-[#00C0FF]/20">Mobile</span>
          </div>
          <p className="text-zinc-500 text-[10px] uppercase font-mono tracking-wider mb-6">Optional portrait uploader specifically configured for phone screens. If left blank, falls back to the main uploader above.</p>
          <div className="flex flex-col md:flex-row items-stretch md:items-start gap-6">
            <div className="w-52 h-44 overflow-hidden rounded-xl border border-white/5 bg-zinc-900/30 shrink-0 relative shadow-sm flex items-center justify-center">
              <img src={sanitizeHeroImage(siteConfig.homeHeroMobileImage || siteConfig.homeHeroImage)} alt="Home Hero Mobile Preview" className="w-full h-full object-cover" />
            </div>
            <div className="flex-grow space-y-4">
              <label className="block w-full cursor-pointer bg-zinc-900/25 border-2 border-dashed border-white/10 hover:border-[#00C0FF] hover:bg-[#00C0FF]/5 rounded-xl p-6 transition-all text-center">
                ...
                <UploadCloud className="w-8 h-8 text-zinc-500 mx-auto mb-2" />
                <p className="text-xs font-bold text-white font-mono uppercase tracking-wider">
                  {isCompressing ? 'Compacting Mobile Backdrop...' : 'Upload Mobile Backdrop'}
                </p>
                <p className="text-[10px] text-zinc-550 font-mono mt-0.5 uppercase tracking-wider">Portrait image (automatically compressed & optimized)</p>
              </label>

              <div className="flex gap-2">
                <input 
                  type="url"
                  value={customHeroMobileUrl}
                  onChange={(e) => setCustomHeroMobileUrl(e.target.value)}
                  placeholder="Or paste direct mobile backdrop image URL..."
                  className="flex-grow text-xs px-4 py-3 border border-white/5 bg-zinc-950 rounded-xl text-white outline-none placeholder-zinc-700 focus:border-[#00C0FF] transition-all font-mono"
                />
                <button 
                  type="button"
                  onClick={() => handleSaveUrl('homeHeroMobileImage', customHeroMobileUrl)}
                  className="bg-[#00C0FF]/25 hover:bg-[#00C0FF] text-[#00C0FF] hover:text-zinc-950 border border-[#00C0FF]/30 font-bold px-4 py-3 rounded-xl text-xs uppercase tracking-widest font-mono transition-all flex items-center shrink-0"
                >
                  <Plus className="w-3.5 h-3.5 mr-1" /> Use URL
                </button>
        ...
        <div>
          <h2 className="text-sm font-bold font-serif text-[#00C0FF] mb-1 uppercase tracking-widest">Client Delivery Wall of Fame</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <div className="space-y-6">
              <div className="border border-white/5 p-6 rounded-2xl bg-zinc-900/20 shadow-lg">
                <h3 className="text-[10px] font-bold text-zinc-400 tracking-widest uppercase mb-4 flex items-center font-mono">
                  <ImageIcon className="w-4 h-4 mr-2 text-[#00C0FF]" />
                  Upload Local File
                </h3>
                <label className="block w-full cursor-pointer bg-zinc-950 border border-dashed border-white/10 hover:border-[#00C0FF] hover:bg-[#00C0FF]/5 rounded-xl p-6 transition-all text-center">
              ...
              <form onSubmit={handleAddDeliveryUrl} className="border border-white/5 p-6 rounded-2xl bg-zinc-900/20 shadow-lg space-y-4">
                <h3 className="text-[10px] font-bold text-zinc-400 tracking-widest uppercase flex items-center font-mono">
                  <LinkIcon className="w-4 h-4 mr-2 text-[#00C0FF]" />
                  Add Image via URL
                </h3>
                <div className="flex gap-2">
                  <input 
                  ...
                  <button 
                    type="submit"
                    className="bg-[#00C0FF] hover:bg-white text-zinc-950 hover:text-zinc-950 font-bold px-4 py-3 rounded-xl text-xs uppercase tracking-widest font-mono transition-all flex items-center shrink-0"
                  >
            ...
            <div className="border border-white/5 p-6 rounded-2xl bg-zinc-950/40 shadow-lg">
              <h3 className="text-[10px] font-bold text-[#00C0FF] tracking-widest uppercase mb-4 font-mono">
        ...
        <div>
          <h2 className="text-sm font-bold font-serif text-[#00C0FF] mb-1 uppercase tracking-widest flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#E1306C]" /> Linked Instagram Reels Feed
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <div className="space-y-6">
              <form onSubmit={handleAddReelUrl} className="border border-white/5 p-6 rounded-2xl bg-zinc-900/20 shadow-lg space-y-4">
                <h3 className="text-[10px] font-bold text-zinc-400 tracking-widest uppercase flex items-center font-mono">
                  <LinkIcon className="w-4 h-4 mr-2 text-[#E1306C]" />
                ...
                <div className="text-[10px] text-zinc-500 space-y-1 font-mono uppercase bg-black/30 p-4 rounded-xl border border-white/5">
                  <p className="text-[#E1306C] font-bold">Supported Formats:</p>
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-grow text-xs px-4 py-3 border border-white/5 bg-zinc-950 rounded-xl text-white outline-none placeholder-zinc-700/60 focus:border-[#E1306C] transition-all font-mono"
                  />
                  <button 
                    type="submit"
                    className="bg-[#E1306C] hover:bg-white text-white hover:text-zinc-950 font-bold px-4 py-3 rounded-xl text-xs uppercase tracking-widest font-mono transition-all flex items-center shrink-0"
                  >
                    <Plus className="w-3.5 h-3.5 mr-1" /> Add
                  </button>
                </div>
            ...
            <div className="border border-white/5 p-6 rounded-2xl bg-zinc-950/40 shadow-lg">
              <h3 className="text-[10px] font-bold text-[#E1306C] tracking-widest uppercase mb-4 font-mono">
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-4 bg-zinc-900/30 border border-white/5 rounded-xl p-3.5 justify-between group">
                        <div className="truncate flex-grow">
                          <p className="text-[8px] font-mono font-bold text-[#E1306C]">REEL #{idx + 1}</p>
                          <p className="text-[11px] font-mono text-zinc-400 truncate mt-0.5">{url}</p>
                          {reelId && (
                            <span className="text-[8px] uppercase tracking-wider font-bold bg-[#E1306C]/10 border border-[#E1306C]/20 text-[#E1306C] px-1.5 py-0.5 rounded mt-1.5 inline-block font-mono">
                              ID: {reelId}
                            </span>
                          )}
                        </div>
