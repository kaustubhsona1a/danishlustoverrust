...
export function toDbPayload(v: any) {
  return {
    ...
    instagram_reel: v.instagramReel || null,
    inspection_notes: v.inspection_notes || v.inspectionNotes || null,
    features: Array.isArray(v.features) ? (
      v.instagramReel ? [...v.features.filter((f: string) => !f.startsWith('instagram_reel:')), `instagram_reel:${v.instagramReel}`] : v.features.filter((f: string) => !f.startsWith('instagram_reel:'))
    ) : (v.instagramReel ? [`instagram_reel:${v.instagramReel}`] : []),
...
export function VehicleProvider({ children }: { children: ReactNode }) {
  ...
  const fetchInventory = async () => {
    const normalizeVehicles = (list: any[]) => {
      return list.map(v => {
        ...
        if (!instagramReel && Array.isArray(features)) {
  ...
  const updateSiteConfig = async (updates: Partial<SiteConfig>) => {
    ...
    if (isAdmin && isSupabaseConfigured()) {
      ...
      const encodedAboutImageWithDeliveries = `${baseAboutImage}|||${deliveriesPart}|||${mobileHeroPart}|||${reelsPart}`;
      ...
      const attemptPayload: any = {
        ...
        company_name: 'Lust Over Rust Pvt. Ltd.',
  ...
  const migrateLocalStorage = async () => {
    const saved = localStorage.getItem('lust_over_rust_vehicles_v2') || localStorage.getItem('jackpot_cars_vehicles_v2');
    if (saved) {
