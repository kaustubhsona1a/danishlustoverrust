...
export default function AdminLayout() {
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-[#00C0FF] font-sans relative">
        <div className="absolute top-[35%] w-[40vw] h-[40vw] bg-[#00C0FF]/5 rounded-full blur-[120px] pointer-events-none mix-blend-screen opacity-45"></div>
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-zinc-300 font-sans relative px-4">
        {/* Soft, noble neon glow in background */}
        ...
        <div className="absolute bottom-[20%] left-[10%] w-[35vw] h-[35vw] bg-[#E1306C]/4 rounded-full blur-[120px] pointer-events-none mix-blend-screen opacity-30"></div>
        ...
        <div className="w-full max-w-md bg-zinc-900/60 backdrop-blur-md border border-zinc-900 rounded-3xl p-10 shadow-2xl relative overflow-hidden text-center z-10">
          <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[#00C0FF]/45 to-transparent"></div>
          
          <Link to="/" className="inline-flex flex-col items-center mb-8 group">
            {siteConfig.logo ? (
              <img src={siteConfig.logo} alt="Lust Over Rust" className="h-14 w-auto object-contain mb-3" />
            ) : (
              <div className="w-14 h-14 bg-zinc-900 border border-zinc-800 rounded-2xl flex items-center justify-center mb-3">
                <CarFront className="w-6 h-6 text-[#00C0FF]" />
              </div>
            )}
            <h1 className="text-xl font-serif text-white tracking-widest leading-none font-bold uppercase block text-center">
              LUST OVER <span className="text-[#00C0FF]">RUST</span>
            </h1>
            <p className="text-[8px] uppercase tracking-[0.45em] text-[#00C0FF] font-mono mt-1.5 font-semibold">Dealer Portal Lock</p>
          </Link>
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            <div className="space-y-2 text-left">
              ...
              <div className="relative">
                <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#00C0FF]" />
                <input 
                  id="dealer-email"
                  type="email" 
                  value={email}
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-zinc-950/80 border border-zinc-800 rounded-xl text-left text-sm font-semibold tracking-wider text-[#00C0FF] focus:outline-none focus:border-[#00C0FF] transition-all"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-2 text-left">
              ...
              <div className="relative">
                <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#00C0FF]" />
                <input 
                  id="dealer-password"
                  type="password" 
                  value={password}
            ...
            <button disabled={loading} type="submit" className="w-full mt-4 bg-[#00C0FF] hover:bg-white disabled:opacity-50 text-zinc-950 py-4 rounded-xl uppercase tracking-widest text-xs font-bold transition-all duration-300 font-mono shadow-md shadow-[#00C0FF]/10">
              {loading ? 'Authenticating...' : 'Unlock Terminal'}
            </button>
          </form>

          <div className="mt-8 pt-6 border-t border-[#00C0FF]/10 flex justify-between items-center text-[10px] tracking-widest uppercase font-semibold font-mono text-zinc-500">
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex relative overflow-hidden font-sans">
      {/* Universal Background Image alignment */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        {sanitizeHeroImage(siteConfig.homeHeroImage) && (
          <img 
            ...
            className="absolute inset-0 w-full h-full object-cover opacity-[0.22] blur-[3px] scale-[1.01]"
          />
        )}
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 w-64 bg-zinc-950/95 lg:bg-zinc-950/65 backdrop-blur-md text-white flex-shrink-0 flex flex-col border-r border-white/5 z-50 lg:z-10 transition-transform duration-300 ease-out transform ${
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-white/5">
          <Link to="/" className="flex items-center space-x-2 group">
            <div className="bg-[#00C0FF] p-1.5 rounded-lg text-zinc-950">
              <CarFront className="w-5 h-5" />
            </div>
            <div className="flex flex-col items-start leading-none">
              <span className="font-serif text-sm tracking-widest text-white font-bold uppercase leading-none">LUST OVER</span>
              <span className="font-mono text-[8px] tracking-[0.3em] font-bold text-[#00C0FF] mt-0.5">RUST</span>
            </div>
          </Link>
        ...
        <div className="flex-1 overflow-y-auto w-full">
          <div className="p-4 flex-grow space-y-1">
            <p className="px-4 text-[9px] font-bold text-[#00C0FF] uppercase tracking-widest mb-4 mt-2 font-mono">Menu</p>
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all font-mono text-xs tracking-wider uppercase font-semibold ${
                    isActive 
                      ? 'bg-[#00C0FF]/15 text-[#00C0FF] border border-[#00C0FF]/25' 
                      : 'text-zinc-400 hover:bg-white/5 hover:text-white border border-transparent'
                  }`}
                >
                  <Icon className={`w-4 h-4 ${isActive ? 'text-[#00C0FF]' : ''}`} />
                  <span>{item.name}</span>
                </Link>
              );
            })}
        ...
        <div className="p-4 border-t border-white/5 bg-black/20">
          <div className="flex items-center p-4 pb-2">
            {user && (user.user_metadata?.avatar_url || user.photoURL) ? (
              <img src={user.user_metadata?.avatar_url || user.photoURL} alt="Admin avatar" className="w-8 h-8 rounded-full mr-3 flex-shrink-0 object-cover border border-[#00C0FF]/40" />
            ) : (
              <div className="w-8 h-8 rounded-full bg-[#00C0FF] flex items-center justify-center text-sm font-bold text-zinc-950 mr-3 flex-shrink-0 font-mono">
                {(user?.user_metadata?.full_name || user?.displayName) ? (user.user_metadata?.full_name || user.displayName)[0].toUpperCase() : 'L'}
              </div>
            )}
            <div className="leading-tight overflow-hidden">
              <p className="text-xs font-semibold truncate text-white" title={user?.user_metadata?.full_name || user?.displayName || user?.email?.split('@')[0] || "Lust Over Rust Partner"}>
                {user?.user_metadata?.full_name || user?.displayName || user?.email?.split('@')[0] || "Lust Over Rust Partner"}
              </p>
              <p className="text-[10px] text-[#00C0FF] font-mono uppercase tracking-wider">
                ● SUPABASE READY
              </p>
            </div>
          </div>
        ...
        <header className="h-16 bg-zinc-950/40 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-4 md:px-8 flex-shrink-0 gap-4">
          <div className="flex items-center space-x-3 flex-1 min-w-0">
            ...
            <div className="hidden sm:flex items-center text-zinc-400 bg-zinc-900/30 px-3 py-1.5 rounded-lg w-full max-w-xs md:max-w-sm border border-white/5">
              <Search className="w-4 h-4 mr-2 text-[#00C0FF] shrink-0" />
              <input 
                type="text" 
                placeholder="Search..." 
            ...
            <div className="sm:hidden flex flex-col items-start leading-none pointer-events-none">
              <span className="font-serif text-[11px] tracking-widest text-white font-bold uppercase">LUST OVER RUST</span>
              <span className="font-mono text-[7px] tracking-[0.2em] font-bold text-[#00C0FF] mt-0.5">DEALER</span>
            </div>
          </div>
          <div className="flex items-center space-x-3 shrink-0">
            <Link to="/" className="text-[10px] sm:text-xs text-[#00C0FF] border border-[#00C0FF]/40 hover:border-[#00C0FF] hover:bg-[#00C0FF]/5 px-3 py-1.5 rounded-lg font-mono font-bold tracking-wider uppercase transition-colors">
              Website ↗
            </Link>
          </div>
        </header>
